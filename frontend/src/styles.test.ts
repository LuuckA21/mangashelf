...
describe('responsive accessibility tokens', () => {
  ...
  it('keeps mobile controls at a thumb-sized 44 pixel minimum', () => {
    ...
    expect(css).toMatch(/button, input, select\s*{\s*min-height: 44px;/)
    expect(css).toMatch(/\.reserve-toggle\s*{\s*width: 44px; height: 44px;/)
    expect(css).toMatch(/\.link-button\s*{[\s\S]*?min-width: 44px;[\s\S]*?min-height: 44px;/)
